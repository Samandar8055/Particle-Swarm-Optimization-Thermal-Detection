...
def init(model, target, name):
    ...
    boxes = [patches.Rectangle((0, 0), 0, 0, linewidth=1, edgecolor='r', facecolor='none') for _ in range(swarm.n_particles)]
    for box in boxes:
        ax2.add_patch(box)
...
if __name__ == "__main__":
    ...
    for name in INFO.keys():
        ...
        print(f"Execution time for {name}: {execution_time} seconds")
